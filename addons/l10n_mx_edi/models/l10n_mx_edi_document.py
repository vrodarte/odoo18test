...
class L10nMxEdiDocument(models.Model):
    ...
    def _get_finkok_credentials(self, company):
        ...
        if company.l10n_mx_edi_pac_test_env:
            return {
                'username': 'AAAAAA\wsTIMBRADOR',
                'password': 'GlMkm9F=b+',
                'sign_url': 'https://test.timbra.mx/WCFTimbrador33/DetecnoPac.svc',
                'cancel_url': 'https://test.timbra.mx/WCFCancelador/CancelacionXML.svc',
            }
        else:
            ...
            return {
                ...
                'sign_url': 'https://www.timbra.mx/WCFTimbrador/DetecnoPac.svc',
                'cancel_url': 'https://www.timbra.mx/WCFCancelador/CancelacionXML.svc',
            }
    ...
    def _finkok_sign(self, credentials, cfdi):
        ...
        #_logger.info('quien llama')
        #_logger.info(curframe)

        #_logger.info('quien llama 2')
        #_logger.info(calframe)
        #_logger.info(calframe[1][3])
    
        #_logger.info('cfdi.decode("utf-8")')
        #_logger.info(cfdi[1])

        strDoc = cfdi.decode("utf-8")
        strDoc = strDoc.replace("<?xml version='1.0' encoding='UTF-8'?>", '')

        baseDoc = etree.XML(strDoc.encode('utf-8'))
        #baseDoc = etree.XML(strDoc)
        #comprobante = baseDoc.find('./Comprobante')
        
        
        _logger.info('baseDoc')
        _logger.info(baseDoc)
        
        _logger.info('baseDoc.attrib[Folio]')
        _logger.info(baseDoc.attrib['Folio'])

        _logger.info('baseDoc.attrib[Serie]')
        _logger.info(baseDoc.attrib['Serie'])

        folio = baseDoc.attrib['Folio']
        serie = baseDoc.attrib['Serie']

        serie_folio = serie+folio

        _logger.info('serie_folio')
        _logger.info(serie_folio)

        _logger.info('self.id')
        _logger.info(self.id)

        _logger.info('self.invoice_ids.ids')
        _logger.info(self.invoice_ids.ids)

        _logger.info('str self.move_id')
        _logger.info(str(self.move_id))

        _logger.info('str self')
        _logger.info(str(self))

        cfdistr = cfdi.decode("utf-8")
        
        linea1 = '<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:a="http://www.w3.org/2005/08/addressing">'
        linea2 ='<s:Header>'
        linea3 = '<a:Action s:mustUnderstand="1">http://tempuri.org/IDetecnoPac/TimbrarCfdi</a:Action>'
        linea4 = '<a:MessageID>urn:uuid:'
        linea5 = serie_folio
        linea6 = '</a:MessageID>'
        linea7 = '<a:ReplyTo>'
        linea8 = '<a:Address>http://www.w3.org/2005/08/addressing/anonymous</a:Address>'
        linea9 = '</a:ReplyTo>'
        linea10 = '<a:To s:mustUnderstand="1">' 
        linea11 = credentials['sign_url']
        linea12 = '</a:To>'
        linea13 = '</s:Header>'
        linea14 = '<s:Body>'
        linea15 = '<TimbrarCfdi xmlns="http://tempuri.org/">'
        linea16 = '<usuario>'
        linea17 = credentials['username']
        linea18 = '</usuario>'
        linea19 = '<password>'
        linea20 = credentials['password']
        linea21 = '</password>'
        linea22 = '<cfdi><![CDATA['
        linea23 = cfdi.decode("utf-8")
        #linea23 = linea23.replace("ESCUELA KEMPER URGATE", "ESCUELA KEMPER URGATE SA DE CV")
        linea24 = ']]></cfdi>'
        linea25 = '</TimbrarCfdi>'
        linea26 = '</s:Body>'
        linea27 = '</s:Envelope>'
            
        data1 = linea1 + linea2 + linea3 + linea4 + linea5 + linea6 + linea7 + linea8 + linea9 + linea10 + linea11 + linea12 + linea13 + linea14 + linea15 + linea16 + linea17 + linea18 + linea19 + linea20 + linea21 + linea22 + linea23 + linea24 + linea25 + linea26 + linea27
        
        _logger.info('invoice_ids')
        _logger.info('data1')
        _logger.info(data1.encode('utf-8'))

        try:
            #transport = Transport(timeout=200)
            ...
            response = requests.post(credentials['sign_url'],
            ...
            if response.status_code != 200:
                ...
                return {
                    'errors': [_("The Detecno service failed to sign with the following error: %s", response.text)],
                }


            #transport = Transport(timeout=1000)
            #session = Session()
            #client = Client('https://test.timbra.mx/WCFTimbrador33/DetecnoPac.svc', transport=Transport(session=session))
            #response = client.service.sendData(data1)
            ...
            return {
                'errors': [_("The Detecno service failed to sign with the following error: %s", str(e))],
            }
        ...
        cfdi_previo = ''.encode('utf-8')

        rootDoc = etree.XML(response.text.encode('utf-8'))

        if '<b:Success>false' in response.text:
            bodyDocCan = rootDoc.find('./s:Body', namespaces={'s':'http://www.w3.org/2003/05/soap-envelope'})
            
            hijos = bodyDocCan.getchildren()

            for hijo in hijos:
                nietos = hijo.getchildren()
                for nieto in nietos:
                    bisnietos = nieto.getchildren()
                    for bisnieto in bisnietos:
                        #_logger.info('bisnieto.tag')
                        #_logger.info(bisnieto.tag)
                        strErrCode = '{http://schemas.datacontract.org/2004/07/Timbrador}ErrCode'
                        strErrDesc = '{http://schemas.datacontract.org/2004/07/Timbrador}ErrDesc'
                        
                        if strErrCode == bisnieto.tag:
                            code = bisnieto.text

                        if strErrDesc == bisnieto.tag:
                            msg = bisnieto.text
        ...
        nsmap = {'b':'http://schemas.datacontract.org/2004/07/Timbrador', 
                 'i':'http://www.w3.org/2001/XMLSchema-instance'}

        #envelopeDoc = etree.SubElement(rootDoc, "Envelope")
        bodyDoc = rootDoc.find('./s:Body', namespaces={'s':'http://www.w3.org/2003/05/soap-envelope'})

        children = bodyDoc.getchildren()
        ...
        for child in children:
            ...
            for grandChild in grandChildren:
                ...
                grandGrandChildren = grandChild.getchildren()

                for grandGrandChild in grandGrandChildren:

                    #_logger.info('grandGrandChild.tag')
                    #_logger.info(grandGrandChild.tag)
                    
                    strComp = '{http://schemas.datacontract.org/2004/07/Timbrador}Comprobante'
                    
                    if strComp == grandGrandChild.tag:
                        _logger.info('grandGrandChild.text')
                        _logger.info(grandGrandChild.text.encode('utf-8'))
                        cfdi_previo = grandGrandChild.text.encode('utf-8')


        
        #_logger.info('bodyDoc')
        #_logger.info(etree.tostring(bodyDoc))

        #timbrarCfdiResponseDoc = rootDoc.find('./TimbrarCfdiResponse', namespaces={'xmlns':'http://tempuri.org/'})

        #_logger.info('timbrarCfdiResponseDoc')
        #_logger.info(timbrarCfdiResponseDoc)

        #comprobante = rootDoc.find('./b:Comprobante',namespaces=nsmap)
        ...
        #cfdi_signed = response.text if 'xml' in response else None
        #if cfdi_signed:
        #    cfdi_signed = cfdi_signed.encode('utf-8')

        _logger.info('cfdi_previo')
        _logger.info(cfdi_previo)
    ...
    def _finkok_cancel(self, cfdi_values, credentials, uuid, cancel_reason, cancel_uuid=None):
        _logger.info('_finkok_cancel 1')

        _logger.info('antes de borrar /home/ubuntu/xmlporfirmar/xmlFirmado.xml ')
        if os.path.exists("/home/ubuntu/xmlporfirmar/xmlFirmado.xml"):
            _logger.info('borra /home/ubuntu/xmlporfirmar/xmlFirmado.xml ')
            os.remove("/home/ubuntu/xmlporfirmar/xmlFirmado.xml")
        else:
            _logger.info('no existe /home/ubuntu/xmlporfirmar/xmlFirmado.xml ')
        ...
        #key_pem = certificate._get_pem_key1(certificate.key)
        fecha_xml = datetime.now()
        fecha_para_cancel = fecha_xml.strftime('%Y-%m-%d\'T\'%H:%M:%S')

        _logger.info('_finkok_cancel 2')

        l_signed_1 = '<Cancelacion xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"'
        l_signed_2 = ' RfcEmisor="'
        l_signed_3 = rfc_emisor
        l_signed_4 = '" Fecha="'
        l_signed_5 = fecha_para_cancel
        l_signed_6 = '" xmlns="http://cancelacfd.sat.gob.mx">'
        l_signed_7 = '<Folios>'
        l_signed_8 = '<Folio Motivo="'
        l_signed_9 = cancel_reason
        l_signed_10 = '"'
        l_signed_11 = ''
        l_signed_12 = ''
        l_signed_13 = ''
        if cancel_uuid:
            l_signed_11 = ' FolioSustitucion="'
            l_signed_12 = cancel_uuid
            l_signed_13 = '"'
        l_signed_14 = ' UUID="'
        l_signed_15 = uuid
        l_signed_16 = '" />'
        l_signed_17 = '</Folios>'
        l_signed_18 = '</Cancelacion>'

        signed_complete = l_signed_1+l_signed_2+l_signed_3+l_signed_4+l_signed_5+l_signed_6+l_signed_7+l_signed_8+l_signed_9+l_signed_10+l_signed_11+l_signed_12+l_signed_13+l_signed_14+l_signed_15+l_signed_16+l_signed_17+l_signed_18

        _logger.info('_finkok_cancel 3')

        #escribe file
        text_file = open("/tmp/paraFirmar.xml", "w")
        text_file.write(signed_complete)
        text_file.close()
        _logger.info('_finkok_cancel 4')

        #escribe cer
        #cer_file = open("/home/ubuntu/xmlporfirmar/cerEKU.cer", "wb")
        #cer_file.write(certificate.content)
        cer_file = open("/home/ubuntu/xmlporfirmar/cerEKU.cer", "w")
        cer_file.write(cer_pem.decode('utf-8'))
        cer_file.close()

        _logger.info('_finkok_cancel 4.1')


        #escribe key
        #key_file = open("/home/ubuntu/xmlporfirmar/keyEKU.key", "wb")
        #key_file.write(certificate.key)
        key_file = open("/home/ubuntu/xmlporfirmar/keyEKU.key", "w")
        key_file.write(key_pem.decode('utf-8'))
        key_file.close()

        _logger.info('_finkok_cancel 4.2')


        #Firma xml
        l1 = 'java -Xmx100m -XX:ReservedCodeCacheSize=64m -XX:-UseCompressedClassPointers -Xss256k -cp "/home/ubuntu/xmlporfirmar/ItOnLineCFDI.jar:/home/ubuntu/xmlporfirmar/commons-beanutils-1.8.3.jar:/home/ubuntu/xmlporfirmar/commons-codec-1.6.jar:/home/ubuntu/xmlporfirmar/commons-collections-3.2.1.jar:/home/ubuntu/xmlporfirmar/commons-digester-1.8.1.jar:/home/ubuntu/xmlporfirmar/commons-io-2.6.jar:/home/ubuntu/xmlporfirmar/commons-lang-2.6.jar:/home/ubuntu/xmlporfirmar/commons-logging-1.1.1.jar:/home/ubuntu/xmlporfirmar/commons-ssl.jar:/home/ubuntu/xmlporfirmar/guava-15.0.jar:/home/ubuntu/xmlporfirmar/itext-pdfa-5.5.0.jar:/home/ubuntu/xmlporfirmar/itextpdf-5.5.0.jar:/home/ubuntu/xmlporfirmar/jasperreports-6.0.0.jar:/home/ubuntu/xmlporfirmar/jaxb-api-2.2.jar:/home/ubuntu/xmlporfirmar/jaxb-core-2.2.11.jar:/home/ubuntu/xmlporfirmar/jaxb-impl-2.2.11.jar:/home/ubuntu/xmlporfirmar/jaxb-xjc-2.2.jar:/home/ubuntu/xmlporfirmar/jdom-2.0.4.jar:/home/ubuntu/xmlporfirmar/log4j-1.2.16.jar:/home/ubuntu/xmlporfirmar/mail-1.4.4.jar:/home/ubuntu/xmlporfirmar/xalan.jar:/home/ubuntu/xmlporfirmar/xerces-2.8.0.jar:/home/ubuntu/xmlporfirmar/zxing-core.jar:/home/ubuntu/xmlporfirmar/zxing-javase.jar" mx.com.itonline.cfdi_core.CancelaDetecno /home/ubuntu/xmlporfirmar/Porfirmar.xml /home/ubuntu/xmlporfirmar/keyEKU.key /home/ubuntu/xmlporfirmar/cerEKU.cer '
        l1_1 = certificate.password
        l1_2 = ' '
        l1_3 = rfc_emisor
        l1_4 = ' '
        l2 = uuid
        l3 = ' '
        l4 = cancel_reason
        l5 = ' /home/ubuntu/xmlporfirmar/xmlFirmado.xml '
        l6 = cancel_uuid
        lcompleto = l1+l1_1+l1_2+l1_3+l1_4+l2+l3+l4+l5
        
        #os.system(lcompleto)
        _logger.info('lcompleto')
        _logger.info(lcompleto)
        resultCmd = subprocess.Popen(lcompleto, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
        #resultCmd = subprocess.call()

        _logger.info('resultCmd')
        _logger.info(resultCmd)

        _logger.info('_finkok_cancel 4.3')

        time.sleep(1)
        
        _logger.info('_finkok_cancel 4.4')
        
        #raw_xml = open('/home/ubuntu/xmlporfirmar/xmlFirmado.xml').read()
        #_logger.info('_finkok_cancel 4.1')
        #xml_root_element = etree.fromstring(raw_xml)
        #_logger.info('_finkok_cancel 4.2')
        #_logger.info('key_pem')
        #_logger.info(key_pem.decode('utf-8'))
        #_logger.info('cer_pem')
        #_logger.info(cer_pem.decode('utf-8'))

        #text_file1 = open('/tmp/key_pem', 'w')
        #text_file1.write(key_pem.decode('utf-8'))
        #text_file1.close()

        #text_file2 = open('/tmp/cer_pem', 'w')
        #text_file2.write(cer_pem.decode('utf-8'))
        #text_file2.close()
        ...
        #_logger.info('_finkok_cancel 5')
        #tree = etree.ElementTree(xml_root_element)
        #tree.write('/tmp/firmado.xml')

        #_logger.info('_finkok_cancel 6')
        
        #Leer xml firmado
        _logger.info('_finkok_cancel 5')
        dataXmlFirmado = ''
        with open('/home/ubuntu/xmlporfirmar/xmlFirmado.xml', 'r') as file:
            dataXmlFirmado = file.read().replace('\n', '')

        dataXmlFirmado = dataXmlFirmado.replace('<?xml version="1.0" encoding="UTF-8" standalone="no"?>', '')
        dataXmlFirmado = dataXmlFirmado.replace('<?xml version="1.0" encoding="UTF-8"?>', '')
        _logger.info('_finkok_cancel 7')

        l_body_1 = '<s:Envelope xmlns:a="http://www.w3.org/2005/08/addressing" xmlns:s="http://www.w3.org/2003/05/soap-envelope">'
        l_body_2 = '<s:Header>'
        l_body_3 = '<a:Action s:mustUnderstand="1">http://tempuri.org/ICanceladorXML/CancelacionCfdiConXML</a:Action>'
        l_body_4 = '<a:MessageID>urn:uuid:'
        l_body_5 = uuid
        l_body_6 = '</a:MessageID>'
        l_body_7 = '<a:ReplyTo>'
        l_body_8 = '<a:Address>http://www.w3.org/2005/08/addressing/anonymous</a:Address>'
        l_body_9 = '</a:ReplyTo>'
        l_body_10 = '<a:To s:mustUnderstand="1">'
        l_body_11 = credentials['cancel_url']
        l_body_12 = '</a:To>'
        l_body_13 = '</s:Header>'
        l_body_14 = '<s:Body>'
        l_body_15 = '<CancelacionCfdiConXML xmlns="http://tempuri.org/">'
        l_body_16 = '<usuario>'
        l_body_17 = credentials['username']
        l_body_18 = '</usuario>'
        l_body_19 = '<password>'
        l_body_20 = credentials['password']
        l_body_21 = '</password>'
        l_body_22 = '<rfcEmisor>'
        l_body_23 = rfc_emisor
        l_body_24 = '</rfcEmisor>'
        l_body_25 = '<xmlCancelacion>'
        #l_body_26 = '<![CDATA["'
        #l_body_27 = dataXmlFirmado
        #l_body_28 = '"]]>'
        l_body_26 = ''
        l_body_27 = html.escape(dataXmlFirmado)
        l_body_28 = ''
        l_body_29 = '</xmlCancelacion>'
        l_body_30 = '</CancelacionCfdiConXML>'
        l_body_31 = '</s:Body>'
        l_body_32 = '</s:Envelope>'

        body_complete = l_body_1+l_body_2+l_body_3+l_body_4+l_body_5+l_body_6+l_body_7+l_body_8+l_body_9+l_body_10+l_body_11+l_body_12+l_body_13+l_body_14+l_body_15+l_body_16+l_body_17+l_body_18+l_body_19+l_body_20+l_body_21+l_body_22+l_body_23+l_body_24+l_body_25+l_body_26+l_body_27+l_body_28+l_body_29+l_body_30+l_body_31+l_body_32

        _logger.info('_finkok_cancel 8')
        _logger.info('body_complete')
        _logger.info(body_complete)

        try:
            response = requests.post(credentials['cancel_url'],
                data=body_complete.encode('utf-8'),
                headers={'Content-Type':'application/soap+xml;charset=UTF-8;',
                            'SOAPAction':'http://tempuri.org/ICanceladorXML/CancelacionCfdiConXML',
                            'Accept':'*/*',
                            'Accept-Encoding':'gzip, deflate, br',
                            'Connection':'keep-alive',
                            'Cache-Control': "no-cache"},
        ...
        msg = None
        if '<b:Success>false' in response.text:
            code = 'Cancelacion no confirmada'
        ...
        if 'Folios' in response and response.Folios:
            ...
            if 'EstatusUUID' in response.Folios.Folio[0]:
                _logger.info('segundo if')
                response_code = response.Folios.Folio[0].EstatusUUID
                if response_code not in ('201', '202'):
                    _logger.info('tercer if')
                    code = response_code
                    msg = _("Cancelling got an error")
        elif 'CodEstatus' in response:
            _logger.info('en el elif')
            code = response.CodEstatus
            msg = _("Cancelling got an error")
        else:
            _logger.info('en el else')
            msg = _('A delay of 2 hours has to be respected before to cancel')
        """
        _logger.info('despues del if')
        errors = []
        _logger.info('Antes del if code')
        if code:
            _logger.info('en el if code')
            errors.append(_("Code : %s", code))
        if msg:
            _logger.info('en el if msg')
            errors.append(_("Message : %s", msg))
        if errors:
            _logger.info('en el if errors')
            return {'errors': errors}

        _logger.info('antes del return')
    ...
    @api.model
    def _get_finkok_credentials_old(self, company):
        ''' Return the company credentials for PAC: finkok. Does not depend on a recordset
        '''
        if company.l10n_mx_edi_pac_test_env:
            return {
    ...
    @api.model
    def _finkok_sign_old(self, credentials, cfdi):
        ''' Send the CFDI XML document to Finkok for signature. Does not depend on a recordset
        '''
    ...
    @api.model
    def _finkok_cancel_old(self, cfdi_values, credentials, uuid, cancel_reason, cancel_uuid=None):
        company = cfdi_values['root_company']
